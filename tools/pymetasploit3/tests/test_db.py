...
def test_hosts(client):
    ...
    assert (
        "created_at" in default_workspace_hosts[0]
    )  # requires that db is connected and you've had a session
...
def test_note_add(client):
    ...
    client.db.workspaces.workspace("default").notes.report(
        type_of_note, data_of_note, update="unique"
    )
...
def test_loot(client):
    client.db.workspaces.workspace("default").loots.report(
        path="pytest", rtype="pytest", data="pytest", host="1.1.1.1"
    )
...
def test_services_add(client):
    client.db.workspaces.workspace().services.report(
        host="1.1.1.3", port=1, proto="tcp"
    )
